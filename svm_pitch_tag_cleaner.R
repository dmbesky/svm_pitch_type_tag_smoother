...
#Load the Data from DB for Illustration Purposes
library(RMySQL)

statcast_db <- dbConnect(MySQL(), user=Sys.getenv('CLUSTER_USERNAME_DB_BIOMECH'),
...
init_data = dbGetQuery(statcast_db, 'SELECT Pitcher as pitcher_id, TaggedPitchType as initial_pitch_type, RelSpeed as velo, InducedVertBreak as vert_break, HorzBreak as horz_break  FROM college_trackman')
init_data <- init_data  %>%
  mutate(initial_pitch_type = as.factor(initial_pitch_type))  #ensure pitch type is a factor
...
test_data = init_data %>% filter(pitcher_id %in% c('Parsons, Hunter', 'Hall, Charles'))

test_data$pitcher_id[test_data$pitcher_id == 'Parsons, Hunter'] <- 'Test Athlete 1'
test_data$pitcher_id[test_data$pitcher_id == 'Hall, Charles'] <- 'Test Athlete 2'

test_data$velo = test_data$velo + rnorm(nrow(test_data), mean = 0, sd = 0.2)
test_data$horz_break = test_data$horz_break + rnorm(nrow(test_data), mean = 0, sd = 0.2)
test_data$vert_break = test_data$vert_break + rnorm(nrow(test_data), mean = 0, sd = 0.2)

test_data$velo = round(test_data$velo, 1)
test_data$horz_break = round(test_data$horz_break, 1)
test_data$vert_break = round(test_data$vert_break, 1)

getwd()

write_csv(test_data, 'sample_data.csv')
